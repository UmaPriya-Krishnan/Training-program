...
def test_insertion(table):
    def test(self):
        ...
        mycursor.execute("select column_name from information_schema.columns where table_name='" + table + "' limit 1")
        data = mycursor.fetchone()
        data = str(data).split(",")[0].replace("(", "").replace("'", "")
        mycursor.execute("select column_name from information_schema.columns where table_name='test_" + table + "'limit 1")
        data1 = mycursor.fetchone()
        data1 = str(data1).split(",")[0].replace("(", "").replace("'", "")
        mycursor.execute("select * from " + schema_test[3] + ".test_" + table + " where " + data1 + " not in (select " + data + " from " + schema_prod[3] + "." + table + ")")
        comp = mycursor.fetchall()
        mycursor.execute("select * from " + schema_prod[3] + "." + table + " where " + data + " not in (select " + data1 + " from "+ schema_test[3] + ".test_" + table + ")")
        comp1 = mycursor.fetchall()
        self.assertEqual(comp == [], True, msg=table + " " + str(comp))
        self.assertEqual(comp1 == [], True, msg=table + " " + str(comp1))
    return test
