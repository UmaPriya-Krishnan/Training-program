    ...
    def fileaccess(self):
        ...
        try:
            ...
            for file_path in sys.path:
                ...
                self.ddl_updates(tables, file_path)
            return sys.path
        ...
        try:
            ...
            for i, j in zip(sqlfile, db_table):
                if i.strip() != j.strip():
                    ...
                    self.obj2.executes("select column_name from information_schema.columns where table_name=" + "'" + table + "'")
...
'''ltables=tables()
for tables in ltables:
    obj=table_creation_updation(tables)
    obj.fileaccess()'''